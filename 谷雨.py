...
class gy(ApiRequest.ApiRequest):
    def __init__(self, data):
        ...
        self.sec.headers = build_weapp_headers(
            ...
            user_agent=('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) '
                        ...
                        'MiniProgramEnv/Windows WindowsWechat/WMPF WindowsWechat(0x63090b19)XWEB/9193'),
        )
    ...
    def login(self):
        ...
        try:
            ...
            log_event('gy_checkin_failed', sid=self.sid, reason=str(exc))
            self.sendmsg += f'❌ 谷雨签到失败：{exc}\n'
            raise

        success = bool(rj.get('success') or rj.get('code') in (0, '0'))
        message = rj.get('msg') or rj.get('message') or rj
        log_event('gy_checkin_result', sid=self.sid, success=success, msg=message)
        if success:
            self.sendmsg += f'✅ 谷雨签到成功：{message}\n'
        else:
            self.sendmsg += f'❌ 谷雨签到失败：{message}\n'
