...
class auou(ApiRequest.ApiRequest):
    def __init__(self, data):
        ...
        self.sec.headers = build_weapp_headers(
            ...
            extra_headers={'Authorization': auth_token, 'companyId': '9'},
        )

    def login(self):
        try:
            resp = request_with_retry(self.sec, 'GET', 'https://credit.app.tuiwe.com/api/v1/fan/sign')
            payload = parse_response_content(resp)
            success, message = normalize_result(payload)
            log_event('auou_checkin_result', success=success, msg=message)
            if success:
                self.sendmsg += f'✅ auou签到成功：{message}\n'
            else:
                self.sendmsg += f'❌ auou签到失败：{message}\n'
        except Exception as exc:  # noqa: BLE001
            log_event('auou_checkin_failed', reason=str(exc))
            self.sendmsg += f'❌ auou签到失败：{exc}\n'
            raise
