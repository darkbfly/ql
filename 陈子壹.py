...
class czy(ApiRequest.ApiRequest):
    def __init__(self, data):
        ...
        self.sec.headers['Extra-Data'] = (
            f'{{"is_weapp":1,"sid":"{self.sid}","version":"2.195.7.101","client":"weapp","bizEnv":"wsc","uuid":"{self.uuid}","ftime":{mytool.getMSecTimestamp()}}}'
        )
    ...
    def login(self):
        ...
        try:
            ...
            log_event('czy_checkin_failed', sid=self.sid, reason=str(exc))
            self.sendmsg += f'❌ 陈子壹签到失败：{exc}\n'
            raise

        success = bool(rj.get('success') or rj.get('code') in (0, '0'))
        message = rj.get('msg') or rj.get('message') or rj
        log_event('czy_checkin_result', sid=self.sid, success=success, msg=message)
        if success:
            self.sendmsg += f'✅ 陈子壹签到成功：{message}\n'
        else:
            self.sendmsg += f'❌ 陈子壹签到失败：{message}\n'
