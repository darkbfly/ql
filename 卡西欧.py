    ...
    def login(self):
        ...
        try:
            resp = request_with_retry(
                self.sec,
                'POST',
                'https://api.casioonline.com.cn/miniprogram/fans-garden/user-sign',
                json=json_data,
            )
            payload = parse_response_content(resp)
            success, message = normalize_result(payload)
            log_event('kxo_checkin_result', success=success, msg=message)
            if success:
                self.sendmsg += f'✅ 卡西欧签到成功：{message}\n'
            else:
                self.sendmsg += f'❌ 卡西欧签到失败：{message}\n'
        except Exception as exc:  # noqa: BLE001
            log_event('kxo_checkin_failed', reason=str(exc))
            self.sendmsg += f'❌ 卡西欧签到失败：{exc}\n'
            raise
