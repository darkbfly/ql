const $ = new Env('飞利浦健康生活签到');
const philips = ($.isNode() ? JSON.parse(process.env.philips || '[]') : $.getjson("philips", []));
let Utils = undefined;
let notice = '';
...
async function main() {
    console.log('飞利浦健康生活签到开始');
    console.log('philips:', JSON.stringify(philips)); // 打印 philips 的内容
    Utils = await loadUtils();
    if (!Array.isArray(philips) || philips.length === 0) {
        console.log('philips 不是一个数组或为空');
        return;
    }
    for (const item of philips) {
        const authorization = item.authorization;
        console.log(`开始签到，authorization: ${authorization}`);
        
        // 签到接口
        let sign = await commonPost('https://www.phlab.philips.com.cn/lab/user/user/sign-in/sign?temp=1722826533610', {}, authorization);
        ...
        // 签到详情接口
        let signInDetail = await commonGet('https://www.phlab.philips.com.cn/lab/user/user/sign-in/sign-in-detail?temp=1722826534083', authorization);
        console.log(`签到详情结果: ${JSON.stringify(signInDetail)}`);
    }
...
async function getCookie() {
    ...
    const index = philips.findIndex(e => e.authorization === newData.authorization);
    if (index !== -1) {
        if (philips[index].authorization === newData.authorization) {
            console.log('Authorization未改变');
            return;
        } else {
            philips[index] = newData;
            console.log('更新authorization:', newData.authorization);
            $.msg($.name, '更新authorization成功!', '');
        }
    } else {
        philips.push(newData);
        console.log('新增authorization:', newData.authorization);
        $.msg($.name, '新增authorization成功!', '');
    }
    $.setjson(philips, "philips");
    console.log('更新后的philips:', JSON.stringify(philips)); // 打印更新后的 philips
}
    ...
    return new Promise(resolve => {
        const options = {
            ...
            headers: {
                ...
                'Referer': 'https://servicewechat.com/wxc80c777f9120bdac/391/page-frame.html',
                'Host': 'www.phlab.philips.com.cn',
...
async function commonGet(url, authorization) {
    return new Promise(resolve => {
        const options = {
            url: url,
            headers: {
