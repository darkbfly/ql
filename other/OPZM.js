...
const opple = ($.isNode() ? JSON.parse(process.env.opple || '[]') : $.getjson("opple", []));
let Utils = undefined;
let notice = '';
...
async function main() {
    console.log('欧普照明签到开始');
    console.log('opple:', JSON.stringify(opple)); // 打印 opple 的内容
    Utils = await loadUtils();
    if (!Array.isArray(opple) || opple.length === 0) {
        console.log('opple 不是一个数组或为空');
        return;
    }
    for (const item of opple) {
        const accessToken = item['Access-Token'];
        console.log(`开始签到，Access-Token: ${accessToken}`);
        
        // 签到接口
        let sign = await commonPost('https://kfscrm.opple.com/opple/scrm/mkt/activities/sign:join', { "activityId": 100000370 }, accessToken);
        console.log(`签到结果: ${JSON.stringify(sign)}`);
        if (sign.code === 200) {
            console.log('签到成功');
            notice += '签到成功\n';
        } else {
            console.log('签到失败:', sign.chnDesc || sign.engDesc);
            notice += `签到失败: ${sign.chnDesc || sign.engDesc}\n`;
        }
    }
...
async function getToken() {
    ...
    if (!accessToken) {
        console.log('未找到Access-Token头部');
        return;
    }
    const newData = { "Access-Token": accessToken };
    console.log('新Access-Token:', newData); // 打印新的 Access-Token
    const index = opple.findIndex(e => e['Access-Token'] === newData['Access-Token']);
    if (index !== -1) {
        if (opple[index]['Access-Token'] === newData['Access-Token']) {
            console.log('Access-Token未改变');
            return;
        } else {
            opple[index] = newData;
            console.log('更新Access-Token:', newData['Access-Token']);
            $.msg($.name, '更新Access-Token成功!', '');
        }
    } else {
        opple.push(newData);
        console.log('新增Access-Token:', newData['Access-Token']);
        $.msg($.name, '新增Access-Token成功!', '');
    }
    $.setjson(opple, "opple");
    console.log('更新后的opple:', JSON.stringify(opple)); // 打印更新后的 opple
}

async function commonPost(url, body, accessToken) {
