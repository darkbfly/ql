const $ = new Env('登康贝比商城签到');
const dengkangbaby = ($.isNode() ? JSON.parse(process.env.dengkangbaby) : $.getjson("dengkangbaby")) || [];
let Utils = undefined;
let notice = '';
...
async function main() {
    console.log('登康贝比商城签到开始');
    Utils = await loadUtils();
    for (const item of dengkangbaby) {
        const token = item.token;
        console.log(`开始签到，token: ${token}`);
        
        // 签到接口
        let sign = await commonPost('https://hx.hemiao100.com/backg/signed/hmSignedActivityInfo/small/centerSigned', 
            {
                "centerId": "bee93334df5eba197967a5d47e65a0f6",
                "appPath": "activity/checkIn/checkIn",
...
async function getCookie() {
    ...
    const index = dengkangbaby.findIndex(e => e.token === newData.token);
    if (index !== -1) {
        if (dengkangbaby[index].token === newData.token) {
            console.log('token未改变');
            return;
        } else {
            dengkangbaby[index] = newData;
            console.log('更新token:', newData.token);
            $.msg($.name, '更新token成功!', '');
        }
    } else {
        dengkangbaby.push(newData);
        console.log('新增token:', newData.token);
        $.msg($.name, '新增token成功!', '');
    }
    $.setjson(dengkangbaby, "dengkangbaby");
}
    ...
    return new Promise(resolve => {
        const options = {
            ...
            headers: {
                ...
                'Referer': 'https://servicewechat.com/wxf7d468db3802b781/221/page-frame.html',
                'Host': 'hx.hemiao100.com',
