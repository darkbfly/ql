...
class lbxb(ApiRequest.ApiRequest):
    def __init__(self, data):
        ...
        self.sec.headers = build_weapp_headers(
            ...
            extra_headers={
                ...
                'unionId': union_id,
                'X-wxde54fd27cb59db51-Token': token,
            },
        )
    ...
    def login(self):
        try:
            ...
            log_event('lbxb_checkin_failed', unionId=self.union_id, reason=str(exc))
            self.sendmsg += f'❌ 立白小白签到失败：{exc}\n'
            raise

        payload = parse_response_content(resp)
        success, message = normalize_result(payload)
        log_event('lbxb_checkin_result', unionId=self.union_id, success=success, msg=message)
        if success:
            self.sendmsg += f'✅ 立白小白签到成功：{message}\n'
        else:
            self.sendmsg += f'❌ 立白小白签到失败：{message}\n'
